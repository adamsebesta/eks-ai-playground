...
- **Webhook vs. polling (added Week 2):** default Argo CD behavior is polling the repo every 3 min, not instant reaction. Real production setups use a GitHub webhook instead — but that requires a *public* endpoint reachable from GitHub's servers, a real tension with defaulting to private networking. Correct pattern: a narrow, deliberate exception — a second, small internet-facing ALB scoped to just the webhook path, everything else (UI/API) stays internal. Build this as its own exercise: second ALB, GitHub webhook config, signature-verification secret, confirm near-instant sync vs. the 3-min baseline.
